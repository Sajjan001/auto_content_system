...
git add .gitignore agents/generate_agent.py push_to_github.bat 2>nul
git rm --cached .env 2>nul || rem ignore
git add .
git commit -m "chore: remove .env and add GROQ_API_KEY runtime check" 2>nul || echo No changes to commit
git branch -M main
